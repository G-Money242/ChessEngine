...
class TestLegality(unittest.TestCase):
    def white_pawn_attack(self):
        b = board.Board('4k3/8/8/8/8/2P5/PP1P1PPP/4K3 w - - 0 1')
        res = b.get_pawn_attacks('w')
        self.assertEqual(res.int,1357840384) # white attack squares

        b = main.Board('4k3/3pp3/1pPPPP1p/P1P3P1/p1p5/1P1P4/8/4K3 w - - 0 1')
        res = b.get_pawn_attacks('b')
        self.assertEqual(res.int, board.wpawn.int)
    
    def king_moves(self):
        fen = '4k3/8/8/8/5r2/2b5/8/4K3 w - - 0 1'
        b = board.Board(fen)
        res = b.get_king_moves('w')
        self.assertEqual(res.int,2064)

        fen = '4k3/8/8/8/2b1r3/8/4K3/8 w - - 0 1'
        b = board.Board(fen)
        res = b.get_king_moves('w')
        self.assertEqual(res.int,267280)
